...
export default function Footer() {
  return (
      <footer className="bg-[#3B2A20] text-cozy-200 mt-20 border-t border-cozy-800">
        <div className="max-w-7xl mx-auto px-4 py-16">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-10 mb-12">
            <div>
              <div className="flex items-center gap-3 mb-4">
                <span className="relative h-9 w-9 overflow-hidden rounded-full bg-cream shadow-cozy">
  <Image
      src="/logo.png"
  ...
  />
              ...
              <p className="text-sm text-cozy-400 leading-relaxed mb-5">
                Curated home, beauty, kitchen, and lifestyle finds for a warm, beautiful, and intentional life.
              </p>
              <div className="flex gap-3">
                <a href="https://www.instagram.com/the_cozy_shelf_official" aria-label="Instagram"
                   className="w-9 h-9 bg-cozy-800 hover:bg-sage-700 rounded-xl flex items-center justify-center transition-colors">
                ...
                <a href="https://in.pinterest.com/the_cozy_shelf_official/" aria-label="Pinterest"
                   className="w-9 h-9 bg-cozy-800 hover:bg-warm-600 rounded-xl flex items-center justify-center transition-colors">
            ...
            {cols.map(col => (
                <div key={col.title}>
                  ...
                  <ul className="space-y-2.5">
                    {col.links.map(l => (
                        <li key={l.href}>
                          <Link href={l.href} className="text-sm text-cozy-400 hover:text-cozy-100 transition-colors">{l.l}</Link>
                        </li>
                    ))}
                  </ul>
                </div>
          ...
          <div className="border-t border-cozy-800 pt-8 flex flex-col sm:flex-row items-center justify-between gap-3">
            <p className="text-xs text-cozy-500">Copyright {new Date().getFullYear()} The Cozy Shelf. Made with warmth.</p>
            <p className="text-xs text-cozy-500">
              We may earn commissions.{' '}
              <Link href="/affiliate-disclosure" className="underline underline-offset-4 hover:text-cozy-300 transition-colors">Disclosure</Link>
            </p>
          </div>
        </div>
      </footer>
